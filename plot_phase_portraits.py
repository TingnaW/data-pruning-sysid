...
def main():
    ...
    y, dot_y = np.meshgrid(
        ...
        np.linspace(dot_y_min, dot_y_max, 30)
    )
    sol = autonomous_dual_stable_equilibria((y, dot_y))
    ddot_y = sol[1]
    plt.streamplot(y, dot_y, dot_y, ddot_y, color="tab:blue", density=1.2, minlength=0.02, maxlength=0.1, linewidth=0.5, arrowsize=0.5)
    fonts = 13
